...
const SITE_URL = 'https://go2-mexico.com';
const DESTINATION_SLUG = 'mexico'; // Used for /thailand-in/january/ style pages
const LOCALES = ['en'];
...
function collectAllUrls() {
  ...
  urls.push({ path: '/chichen-itza-tickets/', priority: '0.9', changefreq: 'monthly' });
  urls.push({ path: '/cancun-catamaran-tour/', priority: '0.9', changefreq: 'monthly' });
  urls.push({ path: '/mexico-city-frida-kahlo-tour/', priority: '0.9', changefreq: 'monthly' });
...
// ---------------------------------------------------------------------------
// Only publish URLs a route can actually render.
//
// This generator builds its list from data/ and content/ without checking that
// a page exists to serve it, which is how these sitemaps came to advertise
// paths like /city/<city>/weather/<month>/ while pages/city/ only contains
// index.tsx and [slug].tsx. Google was being handed hundreds of 404s.
// ---------------------------------------------------------------------------
function collectRoutePatterns() {
  // resolve from this file, so the filter does not depend on whatever
  // the surrounding generator happens to call its project root
  ...
  function walk(dir, prefix) {
    ...
    for (const entry of entries) {
      ...
      if (base.startsWith('_') || base === '404' || base === '500') continue;
      patterns.push(base === 'index' ? (prefix || '/') : prefix + '/' + base);
    }
  }
  walk(pagesDir, '');
  return patterns.map(p => p.split('/').filter(Boolean));
}

function urlHasRoute(urlPath, patterns) {
  const segs = urlPath.split('/').filter(Boolean);
  return patterns.some(pat => {
    const isCatchAll = pat.length && /^\[\.\.\./.test(pat[pat.length - 1]);
    if (isCatchAll) {
      if (segs.length < pat.length - 1) return false;
    } else if (segs.length !== pat.length) {
      return false;
    }
    for (let i = 0; i < pat.length; i++) {
      const p = pat[i];
      if (/^\[/.test(p)) continue;          // dynamic segment matches anything
...
function keepOnlyRoutedUrls(urls) {
  const patterns = collectRoutePatterns();
  const kept = [];
  const dropped = [];
  for (const u of urls) {
    (urlHasRoute(u.path, patterns) ? kept : dropped).push(u);
  }
  if (dropped.length) {
    console.log('\n  Dropped ' + dropped.length + ' URL(s) with no matching page route:');
    const shown = dropped.slice(0, 8).map(d => d.path);
    shown.forEach(p => console.log('    ' + p));
    if (dropped.length > shown.length) {
      console.log('    ... and ' + (dropped.length - shown.length) + ' more');
    }
  }
  return kept;
}
...
function generateSitemap() {
  ...
  const routedUrls = keepOnlyRoutedUrls(urls);
  console.log(`\nCollected ${routedUrls.length} base URLs`);
  ...
  LOCALES.forEach(locale => {
    ...
    const xml = generateSitemapXML(routedUrls, locale);
    ...
    console.log(`  ${sitemapFilename}: ${routedUrls.length} URLs (${locale.toUpperCase()})`);
    totalUrls += routedUrls.length;
  });
